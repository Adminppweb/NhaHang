    ...
    function OrderCreateCtrl($state, $q, orderService, translateService, userService, productService) {
        ...
        vm.addProduct = function () {
            if (!vm.addingProduct.id) {
                toastr.error(translateService.get("Product not found"));
                return;
            }

            if (!vm.customer.id) {
                toastr.error(translateService.get("Please select customer first"));
                return;
            }
        ...
        vm.updateTaxAndShippingPrice = function updateTaxAndShippingPrice() {
            ...
            orderService.updateTaxAndShippingPrice(
                ...
                if (vm.shippingOptions.length === 0) {
                    toastr.error(translateService.get("Sorry, this items can't be shipped to your selected address"));
                } else {
        ...
        vm.createOrder = function createOrder() {
            ...
            orderService.createOrder(
                ...
                vm.showProgress = false;
                toastr.success(translateService.get("Order created"));
