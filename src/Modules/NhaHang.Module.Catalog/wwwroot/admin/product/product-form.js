...
(function ($) {
    angular
        ...
        .controller('ProductFormCtrl', ['$state',
            '$timeout', '$stateParams',
            '$http',
            'categoryService',
            'productService',
    ...
    function ProductFormCtrl($state,
        ...
        vm.product = { isPublished: true, price: 0, isCallForPricing: false, isAllowToOrder: true };
        ...
        vm.thumbnailMedia = null;
        vm.productImages = [];
        vm.productImageMedias = [];
        vm.productDocuments = [];
        vm.productDocumentMedias = [];
        vm.options = [];
        vm.productTemplates = [];
        ...
        vm.copySlug = function () {
            if (vm.product.slug) {
                ...
                inputElement.setAttribute('value', vm.product.slug);
        ...
        vm.tinymceShortDescriptionOptions = {
            menubar: true,
            ...
            toolbar: 'undo redo | blocks styleselect fontselect fontsizeselect | bold italic underline strikethrough | link image media table | align lineheight | numlist bullist indent outdent | emoticons charmap | removeformat',
            max_height: 300,
            min_height: 150,
            max_width: 500,
            min_width: 400
        ...
        vm.tinymceSpecificationOptions = {
            menubar: true,
            plugins: 'anchor autolink charmap codesample emoticons image link lists media searchreplace table visualblocks wordcount code linkchecker preview fullscreen',
            toolbar: 'undo redo | blocks styleselect fontselect fontsizeselect | bold italic underline strikethrough | link image media table | align lineheight | numlist bullist indent outdent | emoticons charmap | removeformat',
            max_height: 300,
        ...
        function onModifyOption(callback) {
            ...
            bootbox.confirm(translateService.get('Add or remove option will clear all existing variations. Are you sure you want to do this?'), function (result) {
                if (result) {
                    $timeout(function () {
                        vm.product.variations = [];
                        callback();
        ...
        //romove imge media Choosed
        vm.removeImageMedia = function removeImageMedia(media) {
            var index = vm.productImageMedias.indexOf(media);
            vm.productImageMedias.splice(index, 1);
        };

        //romove document media Choosed
        vm.removeDocumentMedia = function removeDocumentMedia(media) {
            var index = vm.productDocumentMedias.indexOf(media);
            vm.productDocumentMedias.splice(index, 1);
        };
        ...
        vm.addVariation = function addVariation() {
            ...
            if (!vm.product.variations.find(function (item) { return item.name === variation.name; })) {
                ...
                toastr.error('The ' + variation.name + translateService.get(' has been existing'));
            }
        };
        ...
        vm.save = function save() {
            ...
            vm.product.preview = vm.product.preview === null ? '' : vm.product.preview;
            ...
            if (vm.isEditMode) {
                promise = productService.editProduct(vm.product, vm.thumbnailImage, vm.productImages, vm.productDocuments, vm.thumbnailMedia);
            } else {
            ...
            promise.then(function (response) {
                ...
                if (result.statusCode == 200) {
                    ...
                    $state.go('product', { id: result.result });
                }
                else {
                ...
                .catch(function (response) {
                    ...
                    if (error && angular.isObject(error)) {
                        ...
                        vm.validationErrors.push(translateService.get('Could not add product.'));
                    }
                });
        };
