...
(function () {
    ...
    function ContactListCtrl(contactService, contactAreaService, translateService) {
        ...
        vm.deleteContact = function deleteContact(contact) {
            bootbox.confirm(translateService.get('Are you sure you want to delete this contact: ') + simplUtil.escapeHtml(contact.fullName), function (result) {
                if (result) {
                    contactService.deleteContact(contact)
                       .then(function (result) {
                           vm.getContacts(vm.tableStateRef);
                           toastr.success(contact.name + translateService.get(' has been deleted'));
                       })
