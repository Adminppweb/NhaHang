    ...
    public class UserApiController : Controller
    {
        ...
        public async Task<IActionResult> Post([FromBody] UserForm model)
        {
			...
			try
			{
				...
				{
					await _trackingService.TrackingError("Post user", "BadRequest");
					return Ok("Fail !!!");
				}
				var user = new User
                {
				...
				customJsonResult.Result = user.Id;
				customJsonResult.StatusCode = 200;
			}
			catch (Exception ex)
			{
				await _trackingService.TrackingError("Post user", ex.Message);
        ...
        public async Task<IActionResult> Put(long id, [FromBody] UserForm model)
        {
			...
			{
				_trackingService.TrackingError("PUT user", ex + string.Empty);
				customJsonResult.StatusCode = 502;
			}
			return Ok(customJsonResult);
		}
