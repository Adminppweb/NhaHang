...
(function () {
    ...
    function ThemeListCtrl(themeService, translateService) {
        ...
        vm.useTheme = function useTheme(theme) {
            themeService.useTheme(theme)
                .then(function (result) {
                    ...
                    toastr.success('The ' + theme.displayName + translateService.get(' has been applied'));
                })
        ...
        vm.previewTheme = function previewTheme(theme) {
            ...
            toastr.success('The ' + theme.displayName + translateService.get(' has been set in preview mode'));
        };

        vm.cancelPreviewTheme = function cancelPreviewTheme(theme) {
            window.document.cookie = "theme=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
            vm.previewingTheme = null;
            toastr.success(translateService.get('The previewing of ') + theme.displayName + translateService.get(' has been cancelled.'));
        };
        ...
        vm.deleteTheme = function deleteTheme(theme) {
            bootbox.confirm(translateService.get('Are you sure you want to delete this ') + simplUtil.escapeHtml(theme.name), function (result) {
                if (result) {
                    themeService.deleteTheme(theme.name)
                        .then(function (result) {
                            vm.getThemes();
                            toastr.success(theme.name + translateService.get(' has been deleted'));
                        })
                        .catch(function (response) {
                            toastr.error(response.data.error);
                        });
