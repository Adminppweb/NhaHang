...
(function () {
    ...
    function WidgetInstanceListCtrl(widgetService, translateService) {
        ...
        vm.deleteWidgetInstance = function deleteWidgetInstance(widgetInstance) {
            bootbox.confirm(translateService.get('Are you sure you want to delete this widget: ') + simplUtil.escapeHtml(widgetInstance.name), function (result) {
                if (result) {
                    widgetService.deleteWidgetInstance(widgetInstance.id)
                       .then(function (result) {
                           getWidgetInstances();
                           toastr.success(widgetInstance.name + translateService.get(' has been deleted'));
                       })
