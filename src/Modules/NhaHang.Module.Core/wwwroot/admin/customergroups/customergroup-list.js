...
(function () {
    ...
    function CustomerGroupListCtrl(customergroupService, translateService) {
        ...
        vm.deleteCustomerGroup = function deleteCustomerGroup(customergroup) {
            bootbox.confirm(translateService.get('Are you sure you want to delete this customer group: ') + simplUtil.escapeHtml(customergroup.name), function (result) {
                if (result) {
                    customergroupService.deleteCustomerGroup(customergroup)
                        .then(function (result) {
                            vm.getCustomerGroups(vm.tableStateRef);
                            toastr.success(customergroup.name + translateService.get(' has been deleted'));
                        })
