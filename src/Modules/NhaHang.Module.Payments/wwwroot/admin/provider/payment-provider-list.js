...
(function () {
    ...
    function PaymentProviderListCtrl(paymentProviderService, translateService) {
        ...
        vm.enableProvider = function enableProvider(provider) {
            bootbox.confirm(translateService.get('Are you sure you want to enable this ') + simplUtil.escapeHtml(provider.name) + translateService.get(' provider'), function (result) {
                if (result) {
        ...
        vm.disableProvider = function disableProvider(provider) {
            bootbox.confirm(translateService.get('Are you sure you want to disable this ') + simplUtil.escapeHtml(provider.name) + translateService.get(' provider'), function (result) {
                if (result) {
                    paymentProviderService.disableProvider(provider)
                        .then(function (result) {
                            provider.isEnabled = false;
                            toastr.success(provider.name + translateService.get(' has been disabled'));
                        })
                        .catch(function (response) {
                            toastr.error(response.data.error);
                        });
